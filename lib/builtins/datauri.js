  ...
  att.register('datauri', 'datauri the css or image', function(){
    ...
    **/
    this.transformFile = function(input, output, callback, options){
      ...
      var type = att.file.extname(input).toLowerCase();
      var supportedExtnames = 'cur|png|jpg|jpeg|gif|webp'.split('|');
      var filecontent;
      
      if(supportedExtnames.indexOf(type) !== -1){
        var base64 = fs.readFileSync(input).toString('base64');
        filecontent = 'data:image/' + 
          (type === 'jpg' ? 'jpeg' : type) + ';base64,' + base64;
      }else{
        filecontent = fileutil.read(input, charset);
        options.input = input;
    ...
    this.execute = function(argv, callback){
      var opts = {
        matchFunction: function(name){
          return name.match(/\.(css|less|sass|stylus|cur|png|jpg|jpeg|gif|webp)$/i) && fileutil.isFile(name);
        },
